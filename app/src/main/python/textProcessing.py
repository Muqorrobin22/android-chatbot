...
from datasets import responseText
from forwardChaining import forward_chaining
from disease import list_data_penyakit
...
def textProcessing(input_keluhan):
    ...
    # Fakta penyakit diubah menjadi string untuk diproses lanjut
    # diagnosa_penyakit = ''.join( hasil)



    # for penyakit in list_data_penyakit:
    #     if penyakit['penyakit'] == diagnosa_penyakit:
    #         response = responseText(penyakit = penyakit['penyakit'], jenis= penyakit['jenis'], definisi= penyakit['definisi'], pengobatan= penyakit['pengobatan'], pencegahan= penyakit['pencegahan'], catatan= penyakit['catatan'])
    #
    # return response
